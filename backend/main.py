...
def grade_essay(payload: dict):
    ...
    prompt_lines = []
    if has_essay:
        prompt_lines.append("请以一个资深申论老师的视角，分析这道题目及其材料，给出审题关键点、材料深度解析和作答思路，以及你来作答的话，从哪个或者哪几个材料可以找到什么论点、拟定标题、开头和结尾，同时按照本省要求评析用户的答案，评析答案时需要逐段点评，点评好的地方和需要优化的地方，并优化用户作答思路，给出扣分点，并进行打分，然后按照题目要求给出参考答案以及用户的提升建议，同时给出材料的精彩例句，参考答案尽可能使用材料原词。")
    else:
    ...
    if answer_images:
        ...
        prompt_lines.append("【图片字数判定规则】每行固定为 25 字，总字数=行数*25。若题目有字数要求，而据此估算的总字数与要求相差超过 20%（过多或过少），则视为字数合适、不扣字数分，并在报告中说明为字数判断误差，不因此扣分。")
