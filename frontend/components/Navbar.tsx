    ...
    <nav className="apple-blur sticky top-0 z-[60] border-b border-black/[0.05]">
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        <div className="flex justify-between h-14 md:h-16 items-center">
          <div className="flex items-center space-x-2 md:space-x-3 cursor-pointer group" onClick={() => onNavigate('home')}>
          ...
          <div className="hidden md:flex items-center space-x-8 lg:space-x-10">
            <button onClick={() => onNavigate('home')} className="text-sm font-medium text-[#1d1d1f]/70 hover:text-[#0071e3] transition-colors">题库列表</button>
            <button onClick={() => onNavigate('profile')} className="text-sm font-medium text-[#1d1d1f]/70 hover:text-[#0071e3] transition-colors">历史批改</button>
          </div>
              ...
              <div className="flex items-center space-x-2 md:space-x-3 group relative cursor-pointer">
                ...
                <div className="hidden group-hover:block absolute right-0 top-full pt-2 md:pt-3 w-36 md:w-40 animate-in fade-in slide-in-from-top-2 duration-200">
                  <div className="bg-white rounded-xl md:rounded-2xl border border-black/[0.05] shadow-2xl overflow-hidden py-1">
                    <button onClick={() => onNavigate('profile')} className="flex items-center space-x-2 w-full text-left px-4 py-2 text-sm hover:bg-[#f5f5f7] transition-colors">
                      <i className="far fa-user-circle w-5"></i>
                      <span>个人中心</span>
                    </button>
                    <div className="h-px bg-black/[0.05] mx-3 my-1"></div>
                    <button onClick={onLogout} className="flex items-center space-x-2 w-full text-left px-4 py-2 text-sm hover:bg-[#f5f5f7] transition-colors text-red-500">
